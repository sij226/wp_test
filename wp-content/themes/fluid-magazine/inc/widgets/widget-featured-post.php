...
// register Fluid_Magazine_Featured_Post widget
function fluid_magazine_register_featured_post_widget() {
    register_widget( 'Fluid_Magazine_Featured_Post' );
}
add_action( 'widgets_init', 'fluid_magazine_register_featured_post_widget' );


if ( ! class_exists( 'Fluid_Magazine_Featured_Post' ) ):

 /**
 * Adds Fluid_Magazine_Featured_Post widget.
 */
class Fluid_Magazine_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'fluid_magazine_featured_posts', // Base ID
			__( 'RARA: Featured Post', 'fluid-magazine' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'fluid-magazine' ), ) // Args
		);
	}
                ...
                <?php if( has_post_thumbnail() && $show_thumbnail ){ ?>                    
                    <div class="img-holder">
                        <a href="<?php the_permalink(); ?>">
                            <?php the_post_thumbnail( 'fluid-magazine-featured-post' ); ?>
                        </a>
                    </div>    				
                    <?php } ?>
	...
	public function form( $instance ) {
        ...
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'fluid-magazine' );	      
        $show_thumbnail = !empty( $instance['show_thumbnail'] ) ? $instance['show_thumbnail'] : '' ;        
        $post_list      = !empty( $instance['post_list'] ) ? $instance['post_list'] : 1 ;
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'fluid-magazine' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'fluid-magazine' ); ?></label> 
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = !empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'fluid-magazine' );
...
} // class Fluid_Magazine_Featured_Post
